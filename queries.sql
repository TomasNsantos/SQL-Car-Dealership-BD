...
-- 21. Consulta de veículos disponíveis (não vendidos)
SELECT CHASSI, VALOR, TIPO
FROM AUTO_PLACA
WHERE CHASSI NOT IN (SELECT CHASSI FROM VENDA);

-- 22. Histórico completo de um cliente (vendas e serviços)
SELECT CPF_CLIENTE AS CPF, 'VENDA' AS TIPO, DATA_COMPRA AS DATA
FROM VENDA
UNION ALL
SELECT CPF AS CPF, 'SERVICO' AS TIPO, DT_ENT AS DATA
FROM CONTRATA
ORDER BY CPF, DATA;

-- 23. Total de serviços realizados por carro
SELECT CHASSI, COUNT(*) AS TOTAL_SERVICOS
FROM CONTRATA
GROUP BY CHASSI;

-- 24. Receita total com vendas finalizadas
SELECT SUM(A.VALOR) AS RECEITA_TOTAL
FROM VENDA V
JOIN AUTO_PLACA A ON V.CHASSI = A.CHASSI
WHERE V.STATUS = 'FINALIZADA';

-- 25. Funcionários que nunca realizaram uma venda
SELECT CPF, NOME
FROM FUNCIONARIO
WHERE CPF NOT IN (SELECT CPF_FUNC FROM VENDA);
